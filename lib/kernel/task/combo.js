var fs = require("fs"),
    ...
    parseTemplate = require("./compileTemplate"),
    // 配置文件相关信息
    config,
    // 保存文件相关配置信息
    fileData = {},
...
function scanDir(p) {
    ...
    mwt.fs.walk(baseDir, walkCallback, {
        filter: function (el) {
...
function preCompile() {
    ...
    compileFileList.forEach(function (filePath) {
        fs.readFile(filePath, "utf-8", function (err, data) {
            if(err) {
                mwt.show.showError("reading "+filePath+" error");
                return;
            }
            readCount++;
            ...
            mwt.compileFileData = fileData;
            // 解析文件完毕
            if (readCount == compileFileList.length) {
                // 删除打包目录
                fileUtil.del(path.join(mwt.projectDir, config.javascript.desc || "./publish"));
                analysePackageType();
            }
        });
    });
...
function analysePackageType() {
    ...
    switch(pt) {
        case "all":
            mwt.compileList = generateAllConcatList(ps);
            concat.concatAll(callCompress);
            break;
        case "seperate":
            mwt.compileList = generateSeperateConcatList(ps);
            concat.concatSeperate(callCompress);
            break;
        case "single":
            mwt.compileList = generateSingleList(ps);
...
function generateAllConcatList(ps) {
    // 过滤掉优先列表里的数据
    var filterData = mwt.us.omit(fileData, ps),
        keys = mwt.us.keys(filterData);
...
function compileTemplate(p) {
    parseTemplate.buildPage();
}

function prepareEnv(p, justJs) {
    config = require(path.resolve(p + "\\config.js"));
    // 记录下工程相关的信息
    mwt.projectDir = p;
